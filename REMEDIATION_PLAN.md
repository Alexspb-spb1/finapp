...
## [x] SEC-004 — Перенести создание компании на сервер
...
## [x] SEC-005 — Создать и выполнить backfill membership-документов
...
## [x] SEC-006 — Заменить приглашение с паролем на одноразовое приглашение
...
5. Принятая владельцем фаза 1 (2026-09-06): администратор вручную передаёт
   copy-link; Firebase sendEmailVerification обязателен. Проверка настоящего
   письма и перехода выполняется отдельно после разрешения внешнего пакета.
6. Автоматическая доставка через server-side email provider отложена до
   отдельного решения владельца; при её реализации секрет хранится в secret
   manager/functions secrets. Отложенную доставку не считать реализованной.
...
## [ ] UX-007 — Упорядочить меню по рабочим разделам

Добавлено владельцем 2026-09-06; отдельный PR на этапе UX после UX-006.
Сгруппировать существующие маршруты: Обзор (Дашборд напрямую), Деньги
(Операции, Счета, Календарь, Импорт, Сверка), Отчёты (ДДС, P&L, Бюджет,
Баланс, Прогнозы), Справочники (Контрагенты, Проекты), Управление
(Пользователи, Настройки с учётом capabilities). Активная группа раскрыта,
текущая страница видна; глубина не более двух уровней.

Критерии: desktop/узкий mobile/клавиатура; viewer/accountant/admin;
прямые ссылки, Back/Forward, reload и switch-company сохранены; нет
дублирования маршрутов. Приложить реальные скриншоты локальной синтетической
приёмки и описание до/после. Перенос файлов сам по себе не закрывает задачу.
