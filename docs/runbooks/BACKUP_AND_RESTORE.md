...
использовать/изменять для восстановления **нельзя**.

**Restore-проект создан** (раунд «Часть 4»,
...
| Parent type | `none` (No organization, тот же тип, что у production) |
| Firebase enabled | PASS |
| Billing linked | NO |
| Firestore created | NO |
| Backup bucket created | NO |

Прежние блокеры (`BLOCKED_AUTHENTICATION` из «Часть 2»/«Часть 3») сняты —
...
export/import — это требует отдельных решений владельца по billing
restore-проекта, разделу 4 ниже (`OWNER_ACTION_REQUIRED` по правам доступа
к production для самого export) и явного `PRODUCTION_ACTION_APPROVED` из
`CLAUDE.md`. Сам факт существования restore-проекта **не означает**, что
billing подключён, Firestore создан, bucket создан, либо что
экспорт/восстановление уже выполнялось — ничего из этого не делалось.
