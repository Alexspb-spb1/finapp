...
Этот документ описывает **порядок**, который должен использоваться, когда
`BASE-003` перейдёт из фазы `PRE-FLIGHT` в фазу фактического исполнения —
после того как будут закрыты оба `OWNER_ACTION_REQUIRED` из
`docs/remediation/reports/BASE-003.md`. Сам факт существования этого
runbook **не означает**, что экспорт/восстановление уже выполнялись.
