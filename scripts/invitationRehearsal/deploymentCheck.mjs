...
import { deploymentGuard, deploymentTransport, metadataRequestOptions, runDeploymentCheck } from './deploymentCheckCore.mjs'
...
try {
  ...
  const result = await runDeploymentCheck({
    ...
    get: async ({ url: target, queryParams }) => {
      ...
      return (await client.get(url.pathname, { queryParams, ...metadataRequestOptions(target),
