      ...
      <div className="flex items-center gap-2">
        ...
        {isText ? (
          ...
          >
            ...
            {cond.field === 'categoryId' && categories.filter(c => !c.isGroup).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
    ...
    <div className="flex items-center gap-2">
      ...
      {act.field === 'comment' ? (
        ...
        <select
          ...
          className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-indigo-300 bg-white"
        >
          {act.field === 'categoryId' && categories.filter(c => !c.isGroup).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          {act.field === 'projectId' && (
            <>
