  ...
  const catTrends = useMemo(() => {
    ...
    const byType = (m: string, type: 'expense') => {
      ...
      transactions.filter(t => t.type === type && monthKey(t.date) === m).forEach(t => {
        // Тренд по категориям — операция без категории не относится ни к
        // одной статье, поэтому не участвует в этой разбивке (monthData
        // выше, откуда берутся общие итоги income/expense, не фильтрует
        // по категории).
